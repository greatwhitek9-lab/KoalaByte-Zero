...
This version wires platform drivers where available and falls back to in-module stubs.
Rev 0.5 eye hardware is EYE1, the ESP32-S3 dual-LCD eye board. It does not
instantiate LED_L / LED_R WS2812 ring drivers.
"""
...
class EyeDisplay:
    ...
    def initialize(self):
        logger.info(f"Initialized Eye Display: {self.component.describe()} as EYE1 dual-LCD eyes")
        self.initialized = True
...
class KoalaByteDevice:
    ...
    def __init__(self, components: Optional[Dict[str, HardwareComponent]] = None):
        ...
        if DRIVERS_AVAILABLE:
            ...
            try:
                if EyeDisplayDriver is not None and self.hw_config is not None:
    ...
    def _load_bom_components(self) -> Dict[str, HardwareComponent]:
        ...
        placeholders = ["DS1", "CAM1", "EYE1", "BAT1", "BMS1", "REG1", "F1", "TH1", "TP_BAT", "U_RF1", "U_RF2", "U_RF3", "U_RF4", "GPS1", "SDR1"]
